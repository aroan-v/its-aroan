export const Organizations = [
  {
    category: 'official affiliation',
    title: `Companies I've worked under`,
    description: '',
    entities: [
      {
        name: '',
        description: '',
        alt: '',
        logo: '',
        website: '',
      },
    ],
  },
]

export const pastWorkContent = [
  {
    category: 'Main',
    title: `Companies I've worked under`,
    description: '',
    entities: [
      {
        name: 'Impact Hub Manila',
        alt: 'Impact Hub Manila logo',
        src: '/past-work-logos/main/ihm-logo.png',
      },
      {
        name: 'Innovation Catalyst',
        alt: 'Innovation Catalyst logo',
        src: '/past-work-logos/main/ic-logo.png',
      },
      {
        name: 'Vote Pilipinas',
        alt: 'Vote Pilipinas logo',
        src: '/past-work-logos/main/vp-logo.png',
      },
      {
        name: 'Impact Hub Asia Pacific',
        alt: 'Impact Hub Asia Pacific logo',
        src: '/past-work-logos/main/apac-logo.png',
      },
      {
        name: 'Kuha Sa Tingi',
        alt: 'Kuha Sa Tingi logo',
        src: '/past-work-logos/main/kst-logo.png',
      },
      { name: 'RippleX', alt: 'RippleX logo', src: '/past-work-logos/main/ripplex-logo.png' },
    ],
  },
  {
    category: 'Partners',
    title: `Partners I've Directly Worked With`,
    description: '',
    entities: [
      { name: 'ADB', alt: 'ADB logo', src: '/past-work-logos/partners/adb-logo.png' },
      { name: 'Cartier', alt: 'Cartier logo', src: '/past-work-logos/partners/cartier-logo.png' },
      { name: 'COMELEC', alt: 'COMELEC logo', src: '/past-work-logos/partners/comelec-logo.png' },
      { name: 'KMC', alt: 'KMC logo', src: '/past-work-logos/partners/kmc-logo.png' },
      { name: 'SATO', alt: 'SATO logo', src: '/past-work-logos/partners/sato-logo.png' },
      { name: 'Pasay', alt: 'Pasay City logo', src: '/past-work-logos/ripplex/pasay-logo.png' },
      { name: 'Pasig', alt: 'Pasig City logo', src: '/past-work-logos/ripplex/pasig-logo.png' },
      { name: 'Quezon City', alt: 'Quezon City logo', src: '/past-work-logos/ripplex/qc-logo.png' },
      { name: 'Quimbo', alt: 'Quimbo logo', src: '/past-work-logos/ripplex/quimbo-logo.png' },
      {
        name: 'San Juan City',
        alt: 'San Juan City logo',
        src: '/past-work-logos/ripplex/san-juan-logo.png',
      },
    ],
  },
  {
    category: 'Vote Pilipinas',
    title: `Creative Campaigns I Led for Vote Pilipinas`,
    description: '',
    entities: [
      {
        name: 'Pilipinas Debates 2022: The Turning Point',
        alt: 'Pilipinas Debates 2022: The Turning Point logo',
        src: '/past-work-logos/vp/debates-logo.png',
      },
      {
        name: '#MagparehistroKa',
        alt: '#MagparehistroKa logo',
        src: '/past-work-logos/vp/magparehistro-logo.png',
      },
      {
        name: '#BumotoKa Campaign',
        alt: '#BumotoKa Campaign logo',
        src: '/past-work-logos/vp/bumoto-logo.png',
      },

      { name: 'COMELEC VSO', alt: 'COMELEC VSO logo', src: '/past-work-logos/vp/vso-logo.png' },
    ],
  },
  {
    category: 'Asia Pacific Programs',
    title: `Programs I've directly handled in Impact Hub Asia Pacific Hubs`,
    description: '',
    entities: [
      {
        name: 'Accelerate 2030',
        alt: 'Accelerate 2030 logo',
        src: '/past-work-logos/apac/accelerate-2030-logo.png',
      },
      {
        name: 'Accelerate Membership',
        alt: 'Accelerate Membership logo',
        src: '/past-work-logos/apac/accelerate-membership-logo.png',
      },
      {
        name: 'Generation Impact Fellowship',
        alt: 'Generation Impact Fellowship logo',
        src: '/past-work-logos/apac/gif-logo.png',
      },
      {
        name: 'Hack The Challenge',
        alt: 'Hack The Challenge logo',
        src: '/past-work-logos/apac/htc-logo.png',
      },
      {
        name: 'Humans of Impact',
        alt: 'Humans of Impact logo',
        src: '/past-work-logos/apac/hoi-logo.png',
      },
      {
        name: 'MB Youth Talks',
        alt: 'MB Youth Talks logo',
        src: '/past-work-logos/apac/mb-youth-logo.png',
      },
    ],
  },
  {
    category: 'Impact Hub Programs and Events',
    title: `Programs, Projects, and Events I've Managed`,
    description: '',
    entities: [
      {
        name: '2050 Fest',
        alt: '2050 Fest logo',
        src: '/past-work-logos/programs/2050fest-logo.png',
      },
      {
        name: 'Artisans & Makers',
        alt: 'Artisans & Makers logo',
        src: '/past-work-logos/programs/artisans-logo.png',
      },
      {
        name: `Cartier Women's Initiative`,
        alt: `Cartier Women's Initiative logo`,
        src: '/past-work-logos/programs/cartier-women-logo.png',
      },
      {
        name: 'DigitalXADB',
        alt: 'DigitalXADB logo',
        src: '/past-work-logos/programs/digital-x-adb-logo.png',
      },
      {
        name: 'Elevate Regional Startup Roadshow',
        alt: 'Elevate Regional Startup Roadshow logo',
        src: '/past-work-logos/programs/elevate-logo.png',
      },
      {
        name: 'Elevate Training Program',
        alt: 'Elevate Training Program logo',
        src: '/past-work-logos/programs/elevate-training-program-logo.png',
      },
      {
        name: 'FuckUp Nights',
        alt: 'FuckUp Nights logo',
        src: '/past-work-logos/programs/fun-logo.png',
      },
      {
        name: 'Impact Hackathon 2020',
        alt: 'Impact Hackathon 2020 logo',
        src: '/past-work-logos/programs/hackathon-2020-logo.png',
      },
      {
        name: 'Impact Hackathon 2021',
        alt: 'Impact Hackathon 2021 logo',
        src: '/past-work-logos/programs/hackathon-2021-logo.png',
      },
      {
        name: 'Impact Hackathon 2022',
        alt: 'Impact Hackathon 2022 logo',
        src: '/past-work-logos/programs/hackathon-2022-logo.png',
      },
      { name: 'ICE Hub', alt: 'ICE Hub logo', src: '/past-work-logos/programs/ice-hub-logo.png' },
      {
        name: 'Impact Hackathon',
        alt: 'Impact Hackathon logo',
        src: '/past-work-logos/programs/impact-hackathon-logo.png',
      },
      {
        name: 'Impact2050',
        alt: 'Impact2050 logo',
        src: '/past-work-logos/programs/impact2050-logo.png',
      },
      {
        name: 'Incubate',
        alt: 'Incubate logo',
        src: '/past-work-logos/programs/incubate-logo.png',
      },
      {
        name: 'Plastics Initiative',
        alt: 'Plastics Initiative logo',
        src: '/past-work-logos/programs/plastics-initiative-logo.png',
      },
      { name: 'Prism', alt: 'Prism logo', src: '/past-work-logos/programs/prism-logo.png' },
      {
        name: 'Resiliency Project',
        alt: 'Resiliency Project logo',
        src: '/past-work-logos/programs/resiliency-project-logo.png',
      },
      { name: 'SEED', alt: 'SEED logo', src: '/past-work-logos/programs/seed-logo.png' },
      { name: 'SFI', alt: 'SFI logo', src: '/past-work-logos/programs/sfi-logo.png' },
      {
        name: 'Women Now',
        alt: 'Women Now logo',
        src: '/past-work-logos/programs/women-now-logo.png',
      },
    ],
  },
]
